...
NodeID3.prototype.getTagsFromBuffer = function(filebuffer, options) {
  ...
  let frameSize =
    this.getTagSize(Buffer.from(filebuffer.toString('hex', framePosition, framePosition + 10), 'hex')) + 10;
...
NodeID3.prototype.createPictureFrame = function(data) {
  try {
    ...
    let apicData =
      data instanceof Buffer == true ? Buffer.from(data) : Buffer.from(fs.readFileSync(data, 'binary'), 'binary');
...
NodeID3.prototype.readPictureFrame = function(APICFrame, ID3Version) {
  ...
  if (APICFrame[0] == 0x00) {
    if (ID3Version == 2) {
      ...
      picture.description =
        iconv.decode(
          APICFrame.slice(APICFrame.indexOf(0x00, 1) + 2, APICFrame.indexOf(0x00, APICFrame.indexOf(0x00, 1) + 2)),
          'ISO-8859-1'
        ) || undefined;
...
NodeID3.prototype.readUserDefinedText = function(frame) {
  ...
  if (frame[0] == 0x00) {
    tags = {
      description: iconv
        .decode(frame, 'ISO-8859-1')
        .substring(1, frame.indexOf(0x00, 1))
        .replace(/\0/g, ''),
      value: iconv
        .decode(frame, 'ISO-8859-1')
        .substring(frame.indexOf(0x00, 1) + 1)
